 ...
 * @return rend 0
 */
 //On s'inspire du code de sleep vu en TD3
int tsleep(int pri, void *obs) {
    struct proc* current = &Tproc[GetElecProc()];
    MASK_ALRM; //Masque les IT horloge
    current->p_flag = SLEEP; // p_stats dans le code du TD3
    current->p_ptr = obs;
    cuurent->p_pri = pri;
    UNMASK_ALRM;
    commut(NULL);
